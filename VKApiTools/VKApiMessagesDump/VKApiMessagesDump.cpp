...
public:
	...
	Out& operator<<(bool msg) { UTF8Message += (msg ? "true" : "false"); return *this; }
...
UserInfo GetUserNameById(VK_APIMETHOD& Method, int64_t UserId) {
	...
	if (Answer.IsError || Answer.Object.find("response") == Answer.Object.end()) {
...
static Map::Map<string, string>GetPhotoURL(const json& ite) {
	...
	if (ite.find("sizes") != ite.end() && !ite.at("sizes").is_null() && ite.at("sizes").is_array()) {
		json hataph = ite.at("sizes").get<json>();
		vector<string> sizes = { "base", "w", "z", "y", "x", "r", "q", "p", "o", "m", "s" };
		for (auto& szs : sizes) {
	...
	if (ite.find("orig_photo") != ite.end() && !ite.at("orig_photo").is_null() && ite.at("orig_photo").is_object()) {
		json hataph = ite.at("orig_photo").get<json>();
		Photos["base"] = hataph.at("url").get<string>();
	}
	return Photos;
}
...
static void ReadExtendedInfoProfiles(Map::Map<int64_t, UserInfo>& UsersDataContent, const json& info) {
	...
	if (!items.is_array() || items.is_null() || items.size() == 0)
		return;
...
void PrebuildParseAttachment(VK_APIMETHOD& VkReq, const json& message, list<DocDownloader>& DocDownloads, int64_t DialogId, const VKDialog& Dialog, int Message_Id) {
	...
	bool HasWall = false;
	if (message.find("attachments") != message.end() && !message.at("attachments").is_null()) {
		...
		for (json::iterator st = hata.begin(); st != hata.end(); ++st) {
			...
			if (DocType == "video" || DocType == "wall") {
				...
				if (DocType == "video") {
					...
					dwnldr.Dir = wchar_to_UTF8(FixFileName(Dialog.Title + (!Dialog.IsChat ? L" (Диалог)" : L" (Беседа)")));
			...
			if (DocType == "photo" || DocType == "doc" || DocType == "video" || DocType == "wall" || DocType == "link" || DocType == "album") {
				...
				if (DocType == "photo") {
					...
					if (PhotosUrls.size() <= 0)
						...
						vector<string> vsz = { "q", "r", "x", "y", "z", "w", "base"};
						...
						vector<string> vszm = { "base", "w", "z", "y", "x", "r", "q", "p", "o", "m", "s" };
						for (auto& imgsz : vszm) {
							if (PhotosUrls.exist(imgsz)) {
				...
				if (NeedDoc) {
					...
					dwnldr.Dir = "html\\" + wchar_to_UTF8(FixFileName(Dialog.Title + (!Dialog.IsChat ? L" (Диалог)" : L" (Беседа)")) + L"\\id" + to_wstring(DialogId) + L"\\Материалы");
					dwnldr.CreatedTime = atta.at("date").get<int64_t>();
					dwnldr.DialogTime = message.at("date").get<int64_t>();
					dwnldr.DialogID = DialogId;
					dwnldr.ConversationID = message_id;
...
void BuildHTML_MESSAGE(VK_APIMETHOD& VkReq, const string& UserAgent, Map::Map<int64_t, UserInfo>& UsersDataContent, HTML_MESSAGE_WITH_SUB& html_MESSAGE, const json& message, HTML_PAGE* page, bool is_sub, bool _isOut, list<DocDownloader>& DocDownloads, int64_t DialogId, const VKDialog& Dialog, int Message_Id, bool DownloadAvatar) {
	HTML_MESSAGE* Work_Message = nullptr;
	if (is_sub) {
	...
	if (!UsersDataContent.exist(from_id))
...
void BuildHTMLDialog(const wstring& HTMLParh, time_t Sync, const wstring& RootDir, VK_APIMETHOD& VkReq, const string& JSON, const string& UserAgent, Map::Map<int64_t, UserInfo>& UsersDataContent, const VKDialog& Dialog, int64_t DialogId, list<DocDownloader>& DocDownloads, bool DownloadAvatar)
{
	...
	if (!UsersDataContent.exist(DialogId)) {
		if (Dialog.IsChat)
	...
	try {
		...
		page->add_page_info(Usnm, DownloadAvatar, wchar_to_UTF8(Usnm.pUserName + (!Dialog.IsChat ? L" (Диалог)" : L" (Беседа)")), "https://vk.ru/" + idlink, Usnm.pPhoneNumber, Usnm.pInstagram, Usnm.pSite, PartNum);
...
static void ParseHistorySingle(VK_APIMETHOD& VkReq, int& ParsingOffset, int& ParsingCount, bool& HasError, size_t& DialogsIterators, Map::Map<int64_t, UserInfo>& UsersDataContent, json& DialogData, Map::Pair<int64_t, VKDialog>& i, time_t& dialog_time) {
	...
	do {
		...
		if (DWN.IsError) {
			HasError = true;
			PrintMessage((!i.get_value().IsChat ? string((const char*)u8"Ошибка: диалог с ") : string((const char*)u8"Ошибка: беседа ")) + to_string(DialogsIterators) + " \"" + wchar_to_UTF8(i.get_value().Title) + "!!!", URGB(255, 0, 0));
			break;
		}
		...
		if (ParsingCount == 0) {
			...
			PrintMessage((!i.get_value().IsChat ? string((const char*)u8"Скачивается диалог с ") : string((const char*)u8"Скачивается беседа ")) + to_string(DialogsIterators) + " \"" + wchar_to_UTF8(i.get_value().Title) + (const char*)u8"\" сообщений " + to_string(ParsingCount) + "...", URGB(255, 255, 255));
		}
		else {
			RemoveLastLine();
			PrintMessage((!i.get_value().IsChat ? string((const char*)u8"Скачивается диалог с ") : string((const char*)u8"Скачивается беседа ")) + to_string(DialogsIterators) + " \"" + wchar_to_UTF8(i.get_value().Title) + (const char*)u8"\" сообщений [" + to_string(ParsingOffset) + ":" + to_string(ParsingCount) + "]", URGB(255, 255, 255));
		}
...
void DownloadDialog(const string& access_token, bool avatarsBase64, bool needAlbumsAndDocs) {
	...
	if (!UsersDataContent.exist(UsnmMain.user_id))
	...
	for (Map::Pair<int64_t, VKDialog>& i : Dlgs) {
		...
		if (VkReq.IsDisabled())
			break;
		DialogsIterators++;
		if (!i.get_value().IsChat) {
			if (!UsersDataContent.exist(i.get_key()))
		...
		DialogData << "{\"type\":\"" << (!i.get_value().IsChat ? "dialog" : "chat") << "\", \"" << (!i.get_value().IsChat ? "dialog" : "chat") << "\": ";
		bool HasError = false;
		...
		JSonDialogData.wopen(Path + L"\\" + L"json\\" + FixFileName(i.get_value().Title + (!i.get_value().IsChat ? L" (Диалог)" : L" (Беседа)") + L".json"), OPENType::OPENWrite, true);
		...
		if (!HasError) {
			...
			RemoveLastLine();
			PrintMessage((!i.get_value().IsChat ? string((const char*)u8"Скачивается диалог с ") : string((const char*)u8"Скачивается беседа ")) + to_string(DialogsIterators) + " \"" + wchar_to_UTF8(i.get_value().Title) + (const char*)u8"\" [Сборка HTML...]", URGB(0, 150, 255));

			BuildHTMLDialog(L"html/" + FixFileName(i.get_value().Title + (!i.get_value().IsChat ? L" (Диалог)" : L" (Беседа)")) + L"/id" + to_wstring(i.get_key()) + L"/" + FixFileName(i.get_value().Title + (!i.get_value().IsChat ? L" (Диалог)" : L" (Беседа)")), dialog_time, Path, VkReq, DialogData.GetString(), ANDROID_USERAGENT, UsersDataContent, i.get_value(), i.get_key(), DocDownloads, avatarsBase64);
			RemoveLastLine();
