...
bool Root(RequestParserStruct& Req, const Connection& client) {
	if (Req.http_request["Path"] == L"captcha.png") {
		string cpch;
		cpch.resize(2117);
		memcpy((char*)cpch.data(), Capcha, 2117);
		return SendHTTTPAnswerWithData(client, "200 OK", "image/png", cpch);
	}
	...
	if (wsearch(Req.http_request["Path"], CAPCHA_METHOD_NAME) != wstring::npos) {
		if (!Req.http_get_param.exist("captcha_sid") && !Req.http_post_param.exist("captcha_sid") && !Req.http_get_param.exist("captcha_key") && !Req.http_post_param.exist("captcha_key")) {
			...
			return SendHTTTPAnswerWithData(client, "200 OK", "application/json; charset=utf-8", ansv);
		}
...
/*
	if (wsearch(Req.http_request["Path"], L"audio.get") != wstring::npos) {
		SendHTTTPAnswerWithData(client, "200 OK", "application/json; charset=utf-8", "{\"error\": {\"error_code\": 34, \"error_msg\":\"Deprecated\"}}");
		return;
	}
*/
...
static int parse_post_url_encoded_multipart(const Connection& client, RequestParserStruct& Req, int content_length, multipart& payload) {
	...
	if (std::regex_search(contentType, matches, e)) {
		...
		if (Req.http_request.exist("Content-Encoding")) {
			if (Req.http_request["Content-Encoding"] == L"gzip") {
				subdata = LIB_IMAGE::win_image::decompress_gzip(subdata);
				Req.http_request.add("Content-Length", to_string(subdata.size()));
				Req.bodySize = subdata.size();
			}
			else if (Req.http_request["Content-Encoding"] == L"zstd") {
	...
	if (Req.method == HTTP_METHOD::HTTP_METHOD_POST) {
		if (Req.bodySize > 0) {
			...
			if (status == 0) {
				...
				if (Req.http_request.exist("Content-Encoding")) {
					if (Req.http_request["Content-Encoding"] == L"gzip") {
						http_post_url_encoded = LIB_IMAGE::win_image::decompress_gzip(http_post_url_encoded);
						Req.http_request.add("Content-Length", to_string(http_post_url_encoded.size()));
						Req.bodySize = http_post_url_encoded.size();
					}
					else if (Req.http_request["Content-Encoding"] == L"zstd") {
						http_post_url_encoded = decompress_zstd(http_post_url_encoded);
						Req.http_request.add("Content-Length", to_string(http_post_url_encoded.size()));
						Req.bodySize = http_post_url_encoded.size();
					}
				}
...
THREAD_IMPL(HTTPS_ServerThread, arg) {
	...
	int sslacceptcode = AcceptSSL(client.ssl, true);
	if (client.isSSL()) {
...
void InitVK_Reverser() {
	...
	SSL_CTX* ctx = initialize_ctx(true, true);
	if (!ctx)
		return;
	HTTPSserver_sock = 0;
