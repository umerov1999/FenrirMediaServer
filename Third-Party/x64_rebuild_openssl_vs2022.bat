...
IF NOT EXIST openssl-src-win64-VS2022 GOTO NO_WIN64_SOURCE
DEL openssl-src-win64-VS2022 /Q /F /S
...
cd /d "%CurrentDir%\openssl-src-win64-VS2022"
...
perl Configure VC-WIN64A --release no-shared no-engine no-module no-crypto-mdebug no-tests no-external-tests no-legacy no-weak-ssl-ciphers --openssldir=%CurrentDir%\openssl-src-win64-VS2022/TargetInstall --prefix=%CurrentDir%\openssl-%OPENSSL_VERSION%-64bit-release-static-VS2022 -D_WIN32_WINNT=0x0600
