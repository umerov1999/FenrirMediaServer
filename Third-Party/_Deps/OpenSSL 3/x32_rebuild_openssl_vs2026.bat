...
IF NOT EXIST openssl-src-win32-VS2026 GOTO NO_WIN32_SOURCE
DEL openssl-src-win32-VS2026 /Q /F /S
RMDIR /S /Q openssl-src-win32-VS2026
:NO_WIN32_SOURCE

del openssl-%OPENSSL_VERSION%.tar
%SEVENZIP% e openssl-%OPENSSL_VERSION%.tar.gz
%SEVENZIP% x -y openssl-%OPENSSL_VERSION%.tar
ren openssl-%OPENSSL_VERSION% openssl-src-win32-VS2026
IF EXIST pax_global_header DEL pax_global_header


cd /d "%CurrentDir%\openssl-src-win32-VS2026"
...
perl Configure VC-WIN32 --release no-shared no-module no-crypto-mdebug no-tests no-external-tests no-legacy no-weak-ssl-ciphers no-egd no-uplink no-quic --openssldir=%CurrentDir%\openssl-src-win32-VS2026/TargetInstall --prefix=%CurrentDir%\openssl-%OPENSSL_VERSION%-32bit-release-static-VS2026 -D_WIN32_WINNT=0x0A00
nmake -f Makefile install

cd ../

%SEVENZIP% a -r openssl-%OPENSSL_VERSION%-32bit-release-static-VS2026.7z openssl-%OPENSSL_VERSION%-32bit-release-static-VS2026\*

DEL openssl-%OPENSSL_VERSION%-32bit-release-static-VS2026 /Q /F /S

RMDIR /S /Q openssl-%OPENSSL_VERSION%-32bit-release-static-VS2026
RMDIR /S /Q openssl-src-win32-VS2026
del openssl-%OPENSSL_VERSION%.tar
pause
