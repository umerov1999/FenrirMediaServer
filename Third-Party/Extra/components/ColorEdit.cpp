...
void ColorEdit::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	InsertMenuW(hPopupMenu, indexMenu++, MF_BYPOSITION | (FullText == false ? MF_UNCHECKED : MF_CHECKED), IDC_POPUP_FULL_TEXT, L"Показывать весь текст");
	InsertMenuW(hPopupMenu, indexMenu++, MF_SEPARATOR | MF_BYPOSITION, 0, NULL);
	InsertMenuW(hPopupMenu, indexMenu++, MF_SEPARATOR | MF_BYPOSITION, 0, NULL);
	...
	InsertMenuW(hPopupMenu, indexMenu++, MF_BYPOSITION | (AutoScroll == true ? MF_UNCHECKED : MF_CHECKED), IDC_POPUP_STOP_AUTOSCROLL, L"Остановить прокрутку");
