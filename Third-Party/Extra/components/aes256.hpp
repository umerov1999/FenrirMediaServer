#pragma once

#include <iostream>
#include <cstdlib>
#include <vector>
#include <cstdint>

#define AES_256_BLOCK_SIZE 16
#define AES_256_FE(x)  (((x) << 1) ^ ((((x)>>7) & 1) * 0x1b))
#define AES_256_FD(x)  (((x) >> 1) ^ (((x) & 1) ? 0x8d : 0))

#define AES_256_KEY_SIZE   32
#define AES_256_NUM_ROUNDS 14

class Aes256 {
public:
    Aes256(const std::vector<uint8_t> &key) : m_key(
            std::vector<uint8_t>(key.size() > AES_256_KEY_SIZE ? AES_256_KEY_SIZE : key.size(), 0)),
                                              m_salt(std::vector<uint8_t>(
                                                      AES_256_KEY_SIZE - m_key.size(), 0)),
                                              m_rkey(std::vector<uint8_t>(AES_256_KEY_SIZE, 0)),
                                              m_buffer_pos(0), m_remainingLength(0),
                                              m_decryptInitialized(false) {
        for (std::vector<uint8_t>::size_type i = 0; i < m_key.size(); ++i)
            m_key[i] = key[i];
    }

    static std::vector<uint8_t>::size_type
    encrypt(const std::string &key, const std::vector<uint8_t> &plain,
            std::vector<uint8_t> &encrypted) {
        std::vector<uint8_t> ps;
        ps.resize(key.length());
        memcpy(ps.data(), key.data(), key.length());

        Aes256 aes(ps);

        aes.encrypt_start(plain.size(), encrypted);
        aes.encrypt_continue(plain, encrypted);
        aes.encrypt_end(encrypted);

        return encrypted.size();
    }

    static std::vector<uint8_t>::size_type
    encrypt(const std::string& key, const uint8_t *plain,
            const std::vector<uint8_t>::size_type plain_length,
            std::vector<uint8_t> &encrypted) {
        std::vector<uint8_t> ps;
        ps.resize(key.length());
        memcpy(ps.data(), key.data(), key.length());

        Aes256 aes(ps);

        aes.encrypt_start(plain_length, encrypted);
        aes.encrypt_continue(plain, plain_length, encrypted);
        aes.encrypt_end(encrypted);

        return encrypted.size();
    }

    static std::vector<uint8_t>::size_type
    decrypt(const std::string &key, const std::vector<uint8_t> &encrypted,
            std::vector<uint8_t> &plain) {
        std::vector<uint8_t> ps;
        ps.resize(key.length());
        memcpy(ps.data(), key.data(), key.length());
        Aes256 aes(ps);

        aes.decrypt_start(encrypted.size());
        aes.decrypt_continue(encrypted, plain);
        aes.decrypt_end(plain);

        return plain.size();
    }

    static std::vector<uint8_t>::size_type
    decrypt(const std::string &key, const uint8_t *encrypted,
            const std::vector<uint8_t>::size_type encrypted_length, std::vector<uint8_t> &plain) {
        std::vector<uint8_t> ps;
        ps.resize(key.length());
        memcpy(ps.data(), key.data(), key.length());

        Aes256 aes(ps);

        aes.decrypt_start(encrypted_length);
        aes.decrypt_continue(encrypted, encrypted_length, plain);
        aes.decrypt_end(plain);

        return plain.size();
    }

    std::vector<uint8_t>::size_type
    encrypt_start(const std::vector<uint8_t>::size_type plain_length,
                  std::vector<uint8_t> &encrypted) {
        m_remainingLength = plain_length;

        // Generate salt
        auto it = m_salt.begin(), itEnd = m_salt.end();
        while (it != itEnd)
            *(it++) = (rand() & 0xFF);

        // Calculate padding
        std::vector<uint8_t>::size_type padding = 0;
        if (m_remainingLength % AES_256_BLOCK_SIZE != 0)
            padding = (AES_256_BLOCK_SIZE - (m_remainingLength % AES_256_BLOCK_SIZE));
        m_remainingLength += padding;

        // Add salt
        encrypted.insert(encrypted.end(), m_salt.begin(), m_salt.end());
        m_remainingLength += m_salt.size();

        // Add 1 bytes for padding size
        encrypted.push_back(padding & 0xFF);
        ++m_remainingLength;

        // Reset buffer
        m_buffer_pos = 0;

        return encrypted.size();
    }

    std::vector<uint8_t>::size_type
    encrypt_continue(const std::vector<uint8_t> &plain, std::vector<uint8_t> &encrypted) {
        auto it = plain.begin(), itEnd = plain.end();

        while (it != itEnd) {
            m_buffer[m_buffer_pos++] = *(it++);

            check_and_encrypt_buffer(encrypted);
        }

        return encrypted.size();
    }

    std::vector<uint8_t>::size_type
    encrypt_continue(const uint8_t *plain, const std::vector<uint8_t>::size_type plain_length,
                     std::vector<uint8_t> &encrypted) {
        std::vector<uint8_t>::size_type i = 0;

        while (i < plain_length) {
            m_buffer[m_buffer_pos++] = plain[i++];

            check_and_encrypt_buffer(encrypted);
        }

        return encrypted.size();
    }

    std::vector<uint8_t>::size_type encrypt_end(std::vector<uint8_t> &encrypted) {
        if (m_buffer_pos > 0) {
            while (m_buffer_pos < AES_256_BLOCK_SIZE)
                m_buffer[m_buffer_pos++] = 0;

            encrypt(m_buffer);

            for (m_buffer_pos = 0; m_buffer_pos < AES_256_BLOCK_SIZE; ++m_buffer_pos) {
                encrypted.push_back(m_buffer[m_buffer_pos]);
                --m_remainingLength;
            }

            m_buffer_pos = 0;
        }

        return encrypted.size();
    }

    std::vector<uint8_t>::size_type
    decrypt_start(const std::vector<uint8_t>::size_type encrypted_length) {
        m_remainingLength = encrypted_length;

        // Reset salt
        for (uint8_t j = 0; j < m_salt.size(); ++j)
            m_salt[j] = 0;
        m_remainingLength -= m_salt.size();

        // Reset buffer
        m_buffer_pos = 0;

        m_decryptInitialized = false;

        return m_remainingLength;
    }

    std::vector<uint8_t>::size_type
    decrypt_continue(const std::vector<uint8_t> &encrypted, std::vector<uint8_t> &plain) {
        auto it = encrypted.begin(), itEnd = encrypted.end();
        while (it != itEnd) {
            m_buffer[m_buffer_pos++] = *(it++);

            check_and_decrypt_buffer(plain);
        }
        return plain.size();
    }

    std::vector<uint8_t>::size_type
    decrypt_continue(const uint8_t *encrypted,
                     const std::vector<uint8_t>::size_type encrypted_length,
                     std::vector<uint8_t> &plain) {
        std::vector<uint8_t>::size_type i = 0;
        while (i < encrypted_length) {
            m_buffer[m_buffer_pos++] = encrypted[i++];

            check_and_decrypt_buffer(plain);
        }
        return plain.size();
    }

    static std::vector<uint8_t>::size_type decrypt_end(std::vector<uint8_t> &plain) {
        return plain.size();
    }

private:
    static inline uint8_t rj_xtime(uint8_t x) {
        return (x & 0x80) ? ((x << 1) ^ 0x1b) : (x << 1);
    }

    std::vector<uint8_t> m_key;
    std::vector<uint8_t> m_salt;
    std::vector<uint8_t> m_rkey;

    uint8_t m_buffer[3 * AES_256_BLOCK_SIZE]{};
    uint8_t m_buffer_pos;
    std::vector<uint8_t>::size_type m_remainingLength;

    bool m_decryptInitialized;

    void check_and_encrypt_buffer(std::vector<uint8_t> &encrypted) {
        if (m_buffer_pos == AES_256_BLOCK_SIZE) {
            encrypt(m_buffer);

            for (m_buffer_pos = 0; m_buffer_pos < AES_256_BLOCK_SIZE; ++m_buffer_pos) {
                encrypted.push_back(m_buffer[m_buffer_pos]);
                --m_remainingLength;
            }

            m_buffer_pos = 0;
        }
    }

    void check_and_decrypt_buffer(std::vector<uint8_t> &plain) {
        if (!m_decryptInitialized && m_buffer_pos == m_salt.size() + 1) {
            uint8_t j;
            std::vector<uint8_t>::size_type padding;

            // Get salt
            for (j = 0; j < m_salt.size(); ++j)
                m_salt[j] = m_buffer[j];

            // Get padding
            padding = (m_buffer[j] & 0xFF);
            m_remainingLength -= padding + 1;

            // Start decrypting
            m_buffer_pos = 0;

            m_decryptInitialized = true;
        } else if (m_decryptInitialized && m_buffer_pos == AES_256_BLOCK_SIZE) {
            decrypt(m_buffer);

            for (m_buffer_pos = 0; m_buffer_pos < AES_256_BLOCK_SIZE; ++m_buffer_pos)
                if (m_remainingLength > 0) {
                    plain.push_back(m_buffer[m_buffer_pos]);
                    --m_remainingLength;
                }

            m_buffer_pos = 0;
        }
    }

    void encrypt(uint8_t *buffer) {
        uint8_t i, rcon;

        copy_key();
        add_round_key(buffer, 0);
        for (i = 1, rcon = 1; i < AES_256_NUM_ROUNDS; ++i) {
            sub_bytes(buffer);
            shift_rows(buffer);
            mix_columns(buffer);
            if (!(i & 1))
                expand_enc_key(&rcon);
            add_round_key(buffer, i);
        }
        sub_bytes(buffer);
        shift_rows(buffer);
        expand_enc_key(&rcon);
        add_round_key(buffer, i);
    }

    void decrypt(uint8_t *buffer) {
        uint8_t i, rcon = 1;

        copy_key();
        for (i = AES_256_NUM_ROUNDS / 2; i > 0; --i)
            expand_enc_key(&rcon);

        add_round_key(buffer, AES_256_NUM_ROUNDS);
        shift_rows_inv(buffer);
        sub_bytes_inv(buffer);

        for (i = AES_256_NUM_ROUNDS, rcon = 0x80; --i;) {
            if ((i & 1))
                expand_dec_key(&rcon);
            add_round_key(buffer, i);
            mix_columns_inv(buffer);
            shift_rows_inv(buffer);
            sub_bytes_inv(buffer);
        }
        add_round_key(buffer, i);
    }

    void expand_enc_key(uint8_t *rc) {
        uint8_t i;

        m_rkey[0] = m_rkey[0] ^ sbox[m_rkey[29]] ^ (*rc);
        m_rkey[1] = m_rkey[1] ^ sbox[m_rkey[30]];
        m_rkey[2] = m_rkey[2] ^ sbox[m_rkey[31]];
        m_rkey[3] = m_rkey[3] ^ sbox[m_rkey[28]];
        *rc = AES_256_FE(*rc);

        for (i = 4; i < 16; i += 4) {
            m_rkey[i] = m_rkey[i] ^ m_rkey[i - 4];
            m_rkey[i + 1] = m_rkey[i + 1] ^ m_rkey[i - 3];
            m_rkey[i + 2] = m_rkey[i + 2] ^ m_rkey[i - 2];
            m_rkey[i + 3] = m_rkey[i + 3] ^ m_rkey[i - 1];
        }
        m_rkey[16] = m_rkey[16] ^ sbox[m_rkey[12]];
        m_rkey[17] = m_rkey[17] ^ sbox[m_rkey[13]];
        m_rkey[18] = m_rkey[18] ^ sbox[m_rkey[14]];
        m_rkey[19] = m_rkey[19] ^ sbox[m_rkey[15]];

        for (i = 20; i < 32; i += 4) {
            m_rkey[i] = m_rkey[i] ^ m_rkey[i - 4];
            m_rkey[i + 1] = m_rkey[i + 1] ^ m_rkey[i - 3];
            m_rkey[i + 2] = m_rkey[i + 2] ^ m_rkey[i - 2];
            m_rkey[i + 3] = m_rkey[i + 3] ^ m_rkey[i - 1];
        }
    }

    void expand_dec_key(uint8_t *rc) {
        uint8_t i;

        for (i = 28; i > 16; i -= 4) {
            m_rkey[i + 0] = m_rkey[i + 0] ^ m_rkey[i - 4];
            m_rkey[i + 1] = m_rkey[i + 1] ^ m_rkey[i - 3];
            m_rkey[i + 2] = m_rkey[i + 2] ^ m_rkey[i - 2];
            m_rkey[i + 3] = m_rkey[i + 3] ^ m_rkey[i - 1];
        }

        m_rkey[16] = m_rkey[16] ^ sbox[m_rkey[12]];
        m_rkey[17] = m_rkey[17] ^ sbox[m_rkey[13]];
        m_rkey[18] = m_rkey[18] ^ sbox[m_rkey[14]];
        m_rkey[19] = m_rkey[19] ^ sbox[m_rkey[15]];

        for (i = 12; i > 0; i -= 4) {
            m_rkey[i + 0] = m_rkey[i + 0] ^ m_rkey[i - 4];
            m_rkey[i + 1] = m_rkey[i + 1] ^ m_rkey[i - 3];
            m_rkey[i + 2] = m_rkey[i + 2] ^ m_rkey[i - 2];
            m_rkey[i + 3] = m_rkey[i + 3] ^ m_rkey[i - 1];
        }

        *rc = AES_256_FD(*rc);
        m_rkey[0] = m_rkey[0] ^ sbox[m_rkey[29]] ^ (*rc);
        m_rkey[1] = m_rkey[1] ^ sbox[m_rkey[30]];
        m_rkey[2] = m_rkey[2] ^ sbox[m_rkey[31]];
        m_rkey[3] = m_rkey[3] ^ sbox[m_rkey[28]];
    }

    void sub_bytes(uint8_t *buffer) {
        uint8_t i = AES_256_KEY_SIZE / 2;

        while (i--)
            buffer[i] = sbox[buffer[i]];
    }

    void sub_bytes_inv(uint8_t *buffer) {
        uint8_t i = AES_256_KEY_SIZE / 2;
        while (i--)
            buffer[i] = sboxinv[buffer[i]];
    }

    void copy_key() {
        std::vector<uint8_t>::size_type i;

        for (i = 0; i < m_key.size(); ++i)
            m_rkey[i] = m_key[i];
        for (i = 0; i < m_salt.size(); ++i)
            m_rkey[i + m_key.size()] = m_salt[i];
    }

    void add_round_key(uint8_t *buffer, const uint8_t round) {
        uint8_t i = AES_256_KEY_SIZE / 2;

        while (i--)
            buffer[i] ^= m_rkey[(round & 1) ? i + 16 : i];
    }

    static void shift_rows(uint8_t *buffer) {
        uint8_t i, j, k, l;

        i = buffer[1];
        buffer[1] = buffer[5];
        buffer[5] = buffer[9];
        buffer[9] = buffer[13];
        buffer[13] = i;

        j = buffer[10];
        buffer[10] = buffer[2];
        buffer[2] = j;

        k = buffer[3];
        buffer[3] = buffer[15];
        buffer[15] = buffer[11];
        buffer[11] = buffer[7];
        buffer[7] = k;

        l = buffer[14];
        buffer[14] = buffer[6];
        buffer[6] = l;
    }

    static void shift_rows_inv(uint8_t *buffer) {
        uint8_t i, j, k, l;

        i = buffer[1];
        buffer[1] = buffer[13];
        buffer[13] = buffer[9];
        buffer[9] = buffer[5];
        buffer[5] = i;

        j = buffer[2];
        buffer[2] = buffer[10];
        buffer[10] = j;

        k = buffer[3];
        buffer[3] = buffer[7];
        buffer[7] = buffer[11];
        buffer[11] = buffer[15];
        buffer[15] = k;

        l = buffer[6];
        buffer[6] = buffer[14];
        buffer[14] = l;
    }

    static void mix_columns(uint8_t *buffer) {
        uint8_t i, a, b, c, d, e;

        for (i = 0; i < 16; i += 4) {
            a = buffer[i];
            b = buffer[i + 1];
            c = buffer[i + 2];
            d = buffer[i + 3];

            e = a ^ b ^ c ^ d;

            buffer[i] ^= e ^ rj_xtime(a ^ b);
            buffer[i + 1] ^= e ^ rj_xtime(b ^ c);
            buffer[i + 2] ^= e ^ rj_xtime(c ^ d);
            buffer[i + 3] ^= e ^ rj_xtime(d ^ a);
        }
    }

    static void mix_columns_inv(uint8_t *buffer) {
        uint8_t i, a, b, c, d, e, x, y, z;

        for (i = 0; i < 16; i += 4) {
            a = buffer[i];
            b = buffer[i + 1];
            c = buffer[i + 2];
            d = buffer[i + 3];

            e = a ^ b ^ c ^ d;
            z = rj_xtime(e);
            x = e ^ rj_xtime(rj_xtime(z ^ a ^ c));
            y = e ^ rj_xtime(rj_xtime(z ^ b ^ d));

            buffer[i] ^= x ^ rj_xtime(a ^ b);
            buffer[i + 1] ^= y ^ rj_xtime(b ^ c);
            buffer[i + 2] ^= x ^ rj_xtime(c ^ d);
            buffer[i + 3] ^= y ^ rj_xtime(d ^ a);
        }
    }

    const uint8_t sbox[256] = {
            
    };

    const uint8_t sboxinv[256] = {
            
    };
};