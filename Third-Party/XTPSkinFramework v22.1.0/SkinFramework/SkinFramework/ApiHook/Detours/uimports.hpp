...
static BOOL UPDATE_IMPORTS_XX(HANDLE hProcess, HMODULE hModule, LPCSTR* plpDlls, DWORD nDlls)
{
	...
	DETOUR_TRACE(
		("     Imports: %p..%p\n", (DWORD_PTR)pbModule + inh.IMPORT_DIRECTORY.VirtualAddress,
		 (DWORD_PTR)pbModule + inh.IMPORT_DIRECTORY.VirtualAddress + inh.IMPORT_DIRECTORY.Size));
	...
	_Analysis_assume_(cbNew > sizeof(IMAGE_IMPORT_DESCRIPTOR) * (nDlls + nOldDlls)
