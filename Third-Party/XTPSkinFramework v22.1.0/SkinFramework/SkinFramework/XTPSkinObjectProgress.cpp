...
void CXTPSkinObjectProgress::OnDraw(CDC* pDC)
{
	...
	BOOL bVert	 = (0 != (dwStyle & PBS_VERTICAL));
	...
	if (GetStyle() & PBS_MARQUEE)
	{
		...
		dxBlock		= XTP_DPI_X(pClassProgress->GetThemeInt(0, 0, TMT_PROGRESSCHUNKSIZE,
														XTP_UNDPI_X(rc.Height() * 2 / 3)));
		...
		if (dxBlock == XTP_UNDPI_X(1) && dxSpace == 0)
		{
			...
			{
				CRect rcPart(rcClient.left, rc.top, rcClient.left + rc.right - rcClient.right,
							 rc.bottom);
	...
	dxBlock = XTP_DPI_X(
		pClassProgress->GetThemeInt(0, 0, TMT_PROGRESSCHUNKSIZE, XTP_UNDPI_X(dxBlock)));
	int dxSpace = XTP_DPI_X(pClassProgress->GetThemeInt(0, 0, TMT_PROGRESSSPACESIZE, 2));
...
LRESULT CXTPSkinObjectProgress::OnStartMarquee(WPARAM wParam, LPARAM lParam)
{
	...
	if (bMarquee)
	{
		...
		SetTimer(TID_TIMER, XTPToUIntChecked(m_nMarqueeDelay), NULL);
	}
...
void CXTPSkinObjectProgress::DoStep(int nStep)
{
	...
	DWORD dwStyle   = GetStyle();
	BOOL bVert		= (0 != (dwStyle & PBS_VERTICAL));
	CRect rcMargins = XTP_DPI(
		pClassProgress->GetThemeRect(bVert ? PP_BARVERT : PP_BAR, 0, TMT_SIZINGMARGINS));
