...
CXTPPropExchangeXMLNode::CXTPPropExchangeXMLNode(BOOL bLoading, CXTPDOMNodePtr xmlParentNode,
												 LPCTSTR lpszSection)
{
	m_bLoading		 = bLoading;
...
AFX_INLINE CXTPDOMDocumentPtr PXCreateXmlDocument()
{
	return XTPCreateXmlDocumentInstance(TRUE, &CLSID_XTPDOMDocument,
										(xtpXmlDomDefaultEnabled & ~xtpXmlDomUseXPath)
											| xtpXmlDomUseXSLPattern);
}

BOOL CXTPPropExchangeXMLNode::IsXMLSupported()
{
	static BOOL fXmlSupported = (NULL != PXCreateXmlDocument().GetInterfacePtr());
	return fXmlSupported;
}
...
BOOL CXTPPropExchangeXMLNode::CreateDocumentInstance()
{
	if (m_xmlDocPtr == NULL)
	{
		m_xmlDocPtr = PXCreateXmlDocument();
		if (NULL == m_xmlDocPtr)
		{
...
#	ifdef _DEBUG
	...
	TRACE(_T("Warning: CXTPPropExchangeXMLNode::LoadFromString failed. ParseError.Reason = %s"),
		  (LPCTSTR)strReason);
...
BOOL CXTPPropExchangeXMLNode::LoadFromResource(HINSTANCE hInstance, LPCTSTR lpszResourceName,
...
CString CXTPPropExchangeXMLNode::GetEncodedXML(BOOL bFormatted)
{
	...
	if (IsRootSection())
	{
		...
		if (dwLength > 0 && lpszBuffer[dwLength - 1] <= 13)
			dwLength--; // Remove /n
		if (dwLength > 0 && lpszBuffer[dwLength - 1] <= 13)
			dwLength--; // Remove /r
...
#	define WRITEINDENT                                                                            \
		fileBuffer.Write(lpszBegin,                                                                \
						 XTPToUIntChecked(INT_PTR(lpszCurrent - lpszBegin) - nWhiteSpace));        \
		fileBuffer.Write(lpszCrLf, 2);                                                             \
		for (int j = 0; j < nOffset; j++)                                                          \
			fileBuffer.Write(lpszOffset, 1);                                                       \
	...
	fileBuffer.Write(lpszBegin, XTPToUIntChecked(INT_PTR(lpszCurrent - lpszBegin) - 1));
	fileBuffer.Write(lpszCrLf, 2);
...
void CXTPPropExchangeXMLNode::UpdateDocumentEncoding()
{
	...
	if (SUCCEEDED(m_xmlDocPtr->raw_selectSingleNode(_bstr_t(L"/node()"), &ptrBaseInstruction))
		&& ptrBaseInstruction != 0)
	{
		BSTR bstrNodeName = 0;
	...
	if (!bInstructionExists
		&& SUCCEEDED(m_xmlDocPtr->raw_createProcessingInstruction(_bstr_t(L"xml"),
																  CT2BSTR(strInstruction),
																  &ptrInstruction)))
	{
		CXTPDOMNodePtr xmlNodePtr;
		if (SUCCEEDED(m_xmlDocPtr->raw_selectSingleNode(_bstr_t(L"/node()"), &xmlNodePtr)))
		{
			m_xmlDocPtr->raw_insertBefore(ptrInstruction, _variant_t(xmlNodePtr.GetInterfacePtr()),
										  0);
		}
	}
}
...
CXTPPropExchange* CXTPPropExchangeXMLNode::GetSection(LPCTSTR lpszSection)
{
	...
	CXTPPropExchangeXMLNode* pSection = new CXTPPropExchangeXMLNode(IsLoading(), m_xmlSectionNode,
																	lpszSection);
...
BOOL CXTPPropExchangeXMLNode::ExchangeProp(LPCTSTR pszPropName, VARTYPE vtProp, void* pvProp,
...
public:
	...
	POSITION GetPosition(DWORD dwCount = 0, BOOL /*bCompactMode*/ = TRUE)
	{
		...
		if (m_pContainer->IsLoading())
		{
			...
			CString strNode;
			strNode.Format(_T("%s"),
						   m_strSectionName.IsEmpty() ? _T("*") : (LPCTSTR)m_strSectionName);

			if (FAILED(((CXTPPropExchangeXMLNode*)m_pContainer)
						   ->m_xmlSectionNode->raw_selectNodes(CT2BSTR(strNode), &m_xmlNodes)))
			{
				return NULL;
			}
	...
	{
		CXTPPropExchangeXMLNode* pSection = (CXTPPropExchangeXMLNode*)m_pContainer->GetSection(
			m_strSectionName);
		...
		if (m_pContainer->IsStoring())
		{
			...
			if (xmlElementPtr)
			{
				((CXTPPropExchangeXMLNode*)m_pContainer)
					->m_xmlSectionNode->raw_appendChild(xmlElementPtr, &xmlNodePtr);
			}
