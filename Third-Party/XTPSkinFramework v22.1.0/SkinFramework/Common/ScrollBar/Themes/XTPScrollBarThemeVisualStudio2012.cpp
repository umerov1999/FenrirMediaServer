...
#define GETPARTSTATE(ht, pressed, hot, normal, disabled)                                           \
	(!m_State.bEnabled ? disabled                                                                  \
					   : m_State.nPressedHt == ht ? pressed : m_State.nHotHt == ht ? hot : normal)
...
void CXTPScrollBarThemeVisualStudio2012::DrawScrollBar(CDC* pDC, CXTPScrollBase* pScroll)
{
	...
	switch (pScroll->GetScrollBarStyle())
	{
		...
		case xtpScrollStyleVisualStudio2017:
		case xtpScrollStyleVisualStudio2019:
		case xtpScrollStyleVisualStudio2022:
		default:
		{
														 ...
														 XTPScrollMetricsV* pMetrics)
{
	...
	pDC->FillSolidRect(pMetrics->rcBtnTrack, m_State.nPressedHt == XTP_HTSCROLLTHUMB
												 ? m_State.clrBtnTrackPressed
												 : m_State.nHotHt == XTP_HTSCROLLTHUMB
													   ? m_State.clrBtnTrackHot
														 ...
														 XTPScrollMetricsH* pMetrics)
{
	...
	pDC->FillSolidRect(rcBtnTrack, m_State.nPressedHt == XTP_HTSCROLLTHUMB
									   ? m_State.clrBtnTrackPressed
									   : m_State.nHotHt == XTP_HTSCROLLTHUMB
											 ? m_State.clrBtnTrackHot
											 : m_State.clrBtnTrackEnabled);
