...
int CXTPWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_pwndScrollBar[SB_HORZ]->Create(WS_CHILD | WS_VISIBLE | SBS_HORZ, CRect(0, 0, 0, 0), this,
									 100);
...
void CXTPWnd::OnSize(UINT nType, int cx, int cy)
{
	...
	if (::IsWindow(m_pwndScrollBar[SB_VERT]->GetSafeHwnd()))
	{
		if (bVScroll)
		{
			m_pwndScrollBar[SB_VERT]->MoveWindow(cx - cxVScroll, 0, cxVScroll,
												 cy - (bHScroll ? cyHScroll : 0));
