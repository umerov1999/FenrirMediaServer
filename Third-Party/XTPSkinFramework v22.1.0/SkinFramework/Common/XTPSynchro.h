...
//---------------------------------------------------------------------------
// Summary:
//     Implements no-named simple and lightweight synchronization event primitive.
//---------------------------------------------------------------------------
class _XTP_EXT_CLASS CXTPSimpleEvent
{
	long m_bState; // Current event state (1 - signaled, 0 - non-signaled)
...
public:
	...
	//---------------------------------------------------------------------------
	// Summary:
	//     Determines if an event is in signaled state.
	// Rertuns:
	//     TRUE if an event is in signaled state, FALSE otherwise.
	//---------------------------------------------------------------------------
	BOOL IsSignaled() const;

	//---------------------------------------------------------------------------
	// Summary:
	//     Waits infinitely for an event to enter signaled states.
	//---------------------------------------------------------------------------
	void Wait();
...
{
	long m_nLock;			  // Not 0 if the critical section is locked
	long m_nRecursionCounter; // Recursive lock counter
	DWORD m_dwThreadId;		  // The owner thread's ID
public:
	//---------------------------------------------------------------------------
	// Summary:
...
#	ifdef _DEBUG
	...
	static Diagnostic m_Diagnostic;
#	endif
...
private:
	long m_nSharedLockCounter;	// Number of shared locks obtained.
	long m_nExclusiveLockCounter; // Number of exclusive locks obtained.
	DWORD m_dwExclusiveLockTid;   // Thread ID from where exclusive lock has been obtained.

	CXTPSimpleCriticalSection m_csGeneral; // Internal data access guard.
	CXTPSimpleEvent m_sharedLockEvent;	 // Shared lock event

	static const DWORD m_nLocalSharedLockCounterTlsIdx;
