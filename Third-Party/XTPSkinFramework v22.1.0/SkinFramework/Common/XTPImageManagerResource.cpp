...
XTP_IMAGERESOURCE_INFO* CXTPImageManagerResource::AllocBitmap()
{
	...
	{
		m_hbmpImage = CXTPImageManager::Create32BPPDIBSection(NULL, m_szIcon.cx, m_szIcon.cy,
															  &m_pBits);
	...
	LPBYTE pBits	  = NULL;
	HBITMAP hbmpImage = CXTPImageManager::Create32BPPDIBSection(NULL, m_szIcon.cx + m_nAllocWidth,
																m_szIcon.cy, &pBits);
...
XTP_IMAGERESOURCE_INFO* CXTPImageManagerResource::Add(const CXTPImageManagerIconHandle& hHandle)
{
	...
	if (hHandle.m_hIcon)
	{
		...
		if (CXTPImageManager::IsAlphaIconsImageListSupported()
			&& GetIconInfo(hHandle.m_hIcon, &iconInfo))
		{
			LPBYTE pBitsI   = NULL;
			BOOL bAlpha		= -1;
			HBITMAP hBitmap = CXTPImageManagerIcon::PreMultiplyAlphaBitmap(iconInfo.hbmColor,
																		   &bAlpha, &pBitsI);
		...
		if (m_hbmpImage)
		{
			...
			DrawIconEx(m_dc, pRInfo->rc.left, pRInfo->rc.top, hHandle.m_hIcon, 0, 0, 0, NULL,
					   DI_NORMAL);
			m_dc.SetBitmap(NULL);
		}
...
void CXTPImageManagerResource::CreateIcon(CXTPImageManagerIconHandle& hHandle,
										  XTP_IMAGERESOURCE_INFO* pRInfo)
{
	CXTPLockGuard lock(m_cs);
...
void CXTPImageManagerResource::DrawIconComposited(CDC* pDC, XTP_IMAGERESOURCE_INFO* pRInfo,
												  CPoint pt, CSize szIcon)
{
	CBitmap bmp;
	LPDWORD lpBits = 0;
	bmp.Attach(CXTPImageManager::Create32BPPDIBSection(pDC->GetSafeHdc(), szIcon.cx, szIcon.cy,
													   (LPBYTE*)&lpBits));
	if (!lpBits)
		return;
	...
	{
		m_dc.SetBitmap(m_hbmpMask);
		StretchBlt(dc, 0, 0, szIcon.cx, szIcon.cy, m_dc, pRInfo->rc.left, 0, m_szIcon.cx,
				   m_szIcon.cy, SRCCOPY);
		m_dc.SetBitmap(NULL);
	}
...
void CXTPImageManagerResource::Draw(CDC* pDC, XTP_IMAGERESOURCE_INFO* pRInfo, CPoint pt,
									CSize szIcon, BOOL bDrawComposited)
{
	CXTPLockGuard lock(m_cs);
	...
	if (!pRInfo->bAlpha)
	{
		...
		if (m_hbmpMask)
		{
			...
			StretchBlt(pDC->GetSafeHdc(), pt.x, pt.y, szIcon.cx, szIcon.cy, m_dc, pRInfo->rc.left,
					   0, m_szIcon.cx, m_szIcon.cy, SRCAND);
		...
		if (m_hbmpImage)
		{
			m_dc.SetBitmap(m_hbmpImage);
			StretchBlt(pDC->GetSafeHdc(), pt.x, pt.y, szIcon.cx, szIcon.cy, m_dc, pRInfo->rc.left,
					   0, m_szIcon.cx, m_szIcon.cy, SRCINVERT);
			m_dc.SetBitmap(NULL);
		}
	}
	else
	{
		m_dc.SetBitmap(m_hbmpImage);
		XTPImageManager()->AlphaBlend(pDC->GetSafeHdc(), CRect(pt, szIcon), m_dc,
									  CRect(pRInfo->rc.TopLeft(), m_szIcon));
		m_dc.SetBitmap(NULL);
	}
}
