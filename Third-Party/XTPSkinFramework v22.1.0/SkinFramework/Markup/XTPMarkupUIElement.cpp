...
CXTPMarkupDependencyProperty* CXTPMarkupUIElement::m_pRenderTransformOriginProperty = NULL;
...
void CXTPMarkupUIElement::RegisterMarkupClass()
{
	...
	m_pRenderTransformOriginProperty = CXTPMarkupDependencyProperty::Register(
		L"RenderTransformOrigin", MARKUP_TYPE(CXTPMarkupPoint), MARKUP_TYPE(CXTPMarkupUIElement));
...
CXTPMarkupPoint* CXTPMarkupUIElement::GetRenderTransformOrigin() const
{
	return MARKUP_STATICCAST(CXTPMarkupPoint, GetValue(m_pRenderTransformOriginProperty));
}
	...
	CXTPMarkupUIElement::ApplyRenderTransform(CXTPMarkupDrawingContext* pContext)
{
	...
	if (NULL != pRenderTransform)
	{
		...
		if (NULL != pTransform)
		{
			...
			{
				CXTPMarkupPoint* pt = GetRenderTransformOrigin();
				if (pt)
				{
					// CRect rcFinal = GetFinalRect();
					CSize szDesired = GetDesiredSize();
					pRenderTransform->SetOrigin(szDesired.cx * pt->x, szDesired.cy * pt->y);
				}
