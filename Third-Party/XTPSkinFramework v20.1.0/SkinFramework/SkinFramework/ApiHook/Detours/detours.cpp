...
inline PBYTE detour_skip_jmp(PBYTE pbCode, PVOID* ppGlobals)
{
	...
	{ // movw r12,#xxxx
		...
		{ // movt r12,#xxxx
			...
			{ // ldr  pc,[r12]
				PBYTE pbTarget = (PBYTE)(
					((Opcode2 << 12) & 0xf7000000) | ((Opcode2 << 1) & 0x08000000)
		...
		{ // ldr   x16, [x16, IAT]
			...
			{ // br    x16
				...
				*/
				...
				PBYTE const pbTarget = (PBYTE)((ULONG64)pbCode & 0xfffffffffffff000ULL) + page
									   + offset;
...
static PDETOUR_TRAMPOLINE detour_alloc_trampoline(PBYTE pbTarget)
{
	...
	if (s_pRegion != NULL && s_pRegion->pFree != NULL && s_pRegion->pFree >= pLo
		&& s_pRegion->pFree <= pHi)
	{
	found_region:
...
#	endif // DETOURS_ARM
		...
		else
		{
			...
			DETOUR_TRACE(("detours: pbTarget=%p: "
						  ...
						  o->pbTarget, o->pbTarget[0], o->pbTarget[1], o->pbTarget[2],
						  o->pbTarget[3], o->pbTarget[4], o->pbTarget[5], o->pbTarget[6],
						  o->pbTarget[7], o->pbTarget[8], o->pbTarget[9], o->pbTarget[10],
						  o->pbTarget[11]));
...
#	ifdef DETOURS_ARM64
			PBYTE pbCode   = detour_gen_jmp_indirect(o->pbTarget,
...
#	if DETOUR_DEBUG
	{
		...
		for (n = 0; n < ARRAYSIZE(pTrampoline->rAlign); n++)
		{
			...
			DETOUR_TRACE(
				(" %d/%d", pTrampoline->rAlign[n].obTarget, pTrampoline->rAlign[n].obTrampoline));
		}
		DETOUR_TRACE((" ]\n"));
	}
#	endif
...
#	endif // DETOURS_ARM64
	...
	DETOUR_TRACE(("detours: pbTarget=%p: "
				  ...
				  "%02x %02x %02x %02x "
				  "%02x %02x %02x %02x\n",
				  pbTarget, pbTarget[0], pbTarget[1], pbTarget[2], pbTarget[3], pbTarget[4],
				  pbTarget[5], pbTarget[6], pbTarget[7], pbTarget[8], pbTarget[9], pbTarget[10],
				  pbTarget[11]));
	DETOUR_TRACE(("detours: pbTramp =%p: "
				  "%02x %02x %02x %02x "
				  "%02x %02x %02x %02x "
				  "%02x %02x %02x %02x\n",
				  pTrampoline, pTrampoline->rbCode[0], pTrampoline->rbCode[1],
				  pTrampoline->rbCode[2], pTrampoline->rbCode[3], pTrampoline->rbCode[4],
				  pTrampoline->rbCode[5], pTrampoline->rbCode[6], pTrampoline->rbCode[7],
				  pTrampoline->rbCode[8], pTrampoline->rbCode[9], pTrampoline->rbCode[10],
				  pTrampoline->rbCode[11]));
...
#	ifdef DETOURS_IA64
	...
	pDetour						   = (PBYTE)DetourCodeFromPointer(ppldDetour, &pDetourGlobals);
	PDETOUR_TRAMPOLINE pTrampoline = (PDETOUR_TRAMPOLINE)DetourCodeFromPointer(ppldTrampo,
																			   &pTrampoGlobals);
	DETOUR_TRACE(("  ppldDetour=%p, code=%p [gp=%p]\n", ppldDetour, pDetour, pDetourGlobals));
