...
PBYTE CImageData::Find(REFGUID rguid, DWORD* pcbData)
{
	...
	for (DWORD nOffset = 0; nOffset < m_cbData; nOffset += cbBytes)
	{
		...
		if (pRecord->guid.Data1 == rguid.Data1 && pRecord->guid.Data2 == rguid.Data2
			&& pRecord->guid.Data3 == rguid.Data3 && pRecord->guid.Data4[0] == rguid.Data4[0]
			&& pRecord->guid.Data4[1] == rguid.Data4[1] && pRecord->guid.Data4[2] == rguid.Data4[2]
			&& pRecord->guid.Data4[3] == rguid.Data4[3] && pRecord->guid.Data4[4] == rguid.Data4[4]
			&& pRecord->guid.Data4[5] == rguid.Data4[5] && pRecord->guid.Data4[6] == rguid.Data4[6]
...
public:
	CImageThunks(CImage* pImage, DWORD nThunksMax, DWORD* pnAddr)
	{
		...
		m_pThunks	 = (PIMAGE_THUNK_DATA)m_pImage->AllocateOutput(sizeof(IMAGE_THUNK_DATA)
																	   * nThunksMax,
																   &m_nThunkVirtAddr);
		...
		else
		{
			...
			if (pfSymbolCallback != NULL)
			{
				for (DWORD n = 0; n < pImportFile->m_nImportNames; n++)
				{
					...
					LPCSTR pszName = NULL;
					ULONG nOrdinal = 0;
					if (!(*pfSymbolCallback)(pContext, pImportName->m_nOrig,
											 pImportName->m_nOrdinal, &nOrdinal,
											 pImportName->m_pszOrig, pImportName->m_pszName,
											 &pszName))
					{
						goto fail;
					}
...
BOOL CImage::Write(HANDLE hFile)
{
	...
	for (; n < m_NtHeader.FileHeader.NumberOfSections; n++)
	{
		...
		{
			if (SetFilePointer(hFile, m_SectionHeaders[n].PointerToRawData, NULL, FILE_BEGIN)
				== ~0u)
			{
				return FALSE;
			}
	...
	if (fNeedDetourSection || !m_pImageData->IsEmpty())
	{
		...
		m_SectionHeaders[nSection].Characteristics = IMAGE_SCN_CNT_INITIALIZED_DATA
		...
		//////////////////////////////////////////////////////////////////////////
		//
		if (SetFilePointer(hFile, m_SectionHeaders[nSection].PointerToRawData, NULL, FILE_BEGIN)
			== ~0u)
		{
			return FALSE;
		}
...
BOOL WINAPI DetourBinaryResetImports(_In_ PDETOUR_BINARY pBinary)
{
	...
	return pImage->EditImports(NULL, ResetBywayCallback, ResetFileCallback, ResetSymbolCallback,
							   NULL);
}
