...
//
static BOOL WINAPI LoadNtHeaderFromProcess(HANDLE hProcess, HMODULE hModule,
...
static HMODULE WINAPI EnumerateModulesInProcess(HANDLE hProcess, HMODULE hModuleLast,
												PIMAGE_NT_HEADERS32 pNtHeader)
{
...
static PBYTE FindAndAllocateNearBase(HANDLE hProcess, PBYTE pbModule, PBYTE pbBase, DWORD cbAlloc)
{
	...
	for (;; pbLast = (PBYTE)mbi.BaseAddress + mbi.RegionSize)
	{
		...
		pbAddress = (PBYTE)(((DWORD_PTR)pbAddress + mmGranularityMinusOne)
							& ~mmGranularityMinusOne);
...
{
	if (cchDest == 0 || pszDest == NULL || pszSize == NULL || pszSize[0] == '\0'
							 ...
							 DETOUR_EXE_RESTORE& der)
{
	...
	{
		DETOUR_TRACE(
			("WriteProcessMemory(ish@%p..%p) failed: %d\n", psects, psects + cb, GetLastError()));
		return FALSE;
	}
									   ...
									   _In_ DWORD nDlls)
{
	...
	{
		if (!XTPIsWow64Process(hProcess, &bIs32BitProcess))
		{
			return FALSE;
		}
	}
...
{
	DWORD cbTotal = (sizeof(IMAGE_DOS_HEADER) + sizeof(IMAGE_NT_HEADERS)
					 + sizeof(IMAGE_SECTION_HEADER) + sizeof(DETOUR_SECTION_HEADER)
	...
	DETOUR_TRACE(
		("Copied %d byte payload into target process at %p\n", cbTotal, pbTarget - cbTotal));
	return TRUE;
}
...
VOID CALLBACK DetourFinishHelperProcess(_In_ HWND, _In_ HINSTANCE, _In_ LPSTR, _In_ INT)
{
	...
	{
		DETOUR_TRACE(
			("DetourUpdateProcessWithDll(pid=%d) failed: %d\n", s_pHelper->pid, GetLastError()));
		Result = 9903;
		goto Cleanup;
	}
	Result = 0;
...
#	endif // DETOURS_OPTIONS_BITS
	...
	hr = StringCchPrintfA(szCommand, ARRAYSIZE(szCommand), "rundll32.exe \"%hs\",#1",
						  &helper->rDlls[0]);
	if (!SUCCEEDED(hr))
	{
		goto Cleanup;
	...
	if (pfCreateProcessA(szExe, szCommand, NULL, NULL, FALSE, CREATE_SUSPENDED, NULL, NULL, &si,
						 &pi))
	{
...
#	endif // DETOURS_OPTIONS_BITS
	...
	hr = StringCchPrintfW(szCommand, ARRAYSIZE(szCommand), L"rundll32.exe \"%hs\",#1",
						  &helper->rDlls[0]);
	if (!SUCCEEDED(hr))
	{
		goto Cleanup;
	...
	if (pfCreateProcessW(szExe, szCommand, NULL, NULL, FALSE, CREATE_SUSPENDED, NULL, NULL, &si,
						 &pi))
	{
		if (!DetourCopyPayloadToProcess(pi.hProcess, DETOUR_EXE_HELPER_GUID, helper, helper->cb))
		{
