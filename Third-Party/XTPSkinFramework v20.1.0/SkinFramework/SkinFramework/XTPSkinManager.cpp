...
BOOL CXTPSkinManager::IsWindowFilteredOut(HWND hWnd, LPCTSTR lpszClassName, LPCREATESTRUCT lpcs)
{
	...
	if (0 == lpcs->dwExStyle && 0 == lpcs->style && 0 == lpcs->x && 0 == lpcs->y && 0 == lpcs->cx
		&& 0 == lpcs->cy)
		return TRUE;
	...
	return (NULL != m_pWindowFilter ? m_pWindowFilter->FilterWindow(hWnd, lpszClassName, lpcs)
									: FALSE);
}
...
CMap<HTHEME, HTHEME, CXTPSkinManagerClass*, CXTPSkinManagerClass*>
...
{
	CSingleLock singleLock(&m_themeClassAccess, TRUE);
	CXTPSkinManagerClass* pClass = NULL;
	m_themeClasses.Lookup(hTheme, pClass);
	return pClass;
}
...
void CXTPSkinManagerClass::UnbindAllThemes()
{
	...
	while (NULL != pos)
	{
		HTHEME hTheme				 = NULL;
		CXTPSkinManagerClass* pClass = NULL;
		m_themeClasses.GetNextAssoc(pos, hTheme, pClass);
	...
	while (NULL != pos)
	{
		UnbindTheme(themes.GetNext(pos));
	}
}
