	...
	for (int nIndex = 0; nIndex < nCount; nIndex++)
	{
		...
		if (pItem->IsMDISysButton())
		{
			...
			if (NULL != pClassWindow)
			{
				...
				int nPartID = nID == SC_RESTORE	   ? WP_MDIRESTOREBUTTON
							  : nID == SC_MINIMIZE ? WP_MDIMINBUTTON
												   : WP_MDICLOSEBUTTON;
...
void CXTPSkinPopupMenu::OnPaint()
{
	...
	for (int nIndex = m_scrollInfo.nScrollFirst; nIndex < nCount; nIndex++)
	{
		...
		if (pItem->IsOwnerDraw())
		{
			...
			{
				COLORREF clr  = pMetrics->GetColor(!bEnabled   ? COLOR_GRAYTEXT
												   : bSelected ? COLOR_HIGHLIGHTTEXT
															   : COLOR_MENUTEXT);
		...
		else
		{
			...
			dc.SetTextColor(pMetrics->GetColor(!bEnabled   ? COLOR_GRAYTEXT
											   : bSelected ? COLOR_HIGHLIGHTTEXT
														   : COLOR_MENUTEXT));
		}
