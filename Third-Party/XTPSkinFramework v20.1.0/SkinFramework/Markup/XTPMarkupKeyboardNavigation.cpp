...
{
	m_pIsTabStopProperty = CXTPMarkupDependencyProperty::RegisterAttached(
		L"IsTabStop", MARKUP_TYPE(CXTPMarkupBool), MARKUP_TYPE(CXTPMarkupKeyboardNavigation),
		new CXTPMarkupPropertyMetadata(CXTPMarkupBool::CreateTrueValue()));

	m_pTabNavigationProperty = CXTPMarkupDependencyProperty::RegisterAttached(
		L"TabNavigation", MARKUP_TYPE(CXTPMarkupEnum), MARKUP_TYPE(CXTPMarkupKeyboardNavigation),
		new CXTPMarkupPropertyMetadata(
			NULL, &CXTPMarkupKeyboardNavigation::ConvertKeyboardNavigationMode));

	m_pTabIndexProperty = CXTPMarkupDependencyProperty::RegisterAttached(
		L"TabIndex", MARKUP_TYPE(CXTPMarkupInt), MARKUP_TYPE(CXTPMarkupKeyboardNavigation),
...
XTPMarkupKeyboardNavigationMode
	CXTPMarkupKeyboardNavigation::GetTabNavigation(CXTPMarkupObject* pElement)
{
	CXTPMarkupEnum* pValue = MARKUP_STATICCAST(CXTPMarkupEnum,
											   pElement->GetValue(m_pTabNavigationProperty));
	return pValue ? (XTPMarkupKeyboardNavigationMode)(int)*pValue
	...
	CXTPMarkupDependencyProperty* /*pAssociatedProperty = NULL*/)
{
	if (IsStringObject(pObject))
	{
		...
		if (nLength == 8 && _wcsicmp(lpszValue, L"Continue") == 0)
			return CXTPMarkupEnum::CreateValue(xtpMarkupKeyboardNavigationContinue);
		if (nLength == 4 && _wcsicmp(lpszValue, L"Once") == 0)
			return CXTPMarkupEnum::CreateValue(xtpMarkupKeyboardNavigationOnce);
		if (nLength == 5 && _wcsicmp(lpszValue, L"Cycle") == 0)
			return CXTPMarkupEnum::CreateValue(xtpMarkupKeyboardNavigationCycle);
		if (nLength == 4 && _wcsicmp(lpszValue, L"None") == 0)
			return CXTPMarkupEnum::CreateValue(xtpMarkupKeyboardNavigationNone);
		if (nLength == 9 && _wcsicmp(lpszValue, L"Contained") == 0)
			return CXTPMarkupEnum::CreateValue(xtpMarkupKeyboardNavigationContained);
		if (nLength == 5 && _wcsicmp(lpszValue, L"Local") == 0)
			return CXTPMarkupEnum::CreateValue(xtpMarkupKeyboardNavigationLocal);
	}

	return NULL;
}

BOOL CXTPMarkupKeyboardNavigation::OnWndMsg(UINT /*message*/, WPARAM /*wParam*/, LPARAM /*lParam*/,
											LRESULT* /*pResult*/)
{
	// NotImplmeneted
	return FALSE;
...
XTPMarkupKeyboardNavigationMode
	CXTPMarkupKeyboardNavigation::GetKeyNavigationMode(CXTPMarkupObject* pElement)
{
	CXTPMarkupEnum* pValue = MARKUP_STATICCAST(CXTPMarkupEnum,
											   pElement->GetValue(m_pNavigationProperty));
	return pValue ? (XTPMarkupKeyboardNavigationMode)(int)*pValue
				  : xtpMarkupKeyboardNavigationContinue;
}
...
CXTPMarkupObject* CXTPMarkupKeyboardNavigation::GetGroupParent(CXTPMarkupObject* e,
															   BOOL bIncludeCurrent)
{
...
CXTPMarkupObject* CXTPMarkupKeyboardNavigation::GetNextInTree(CXTPMarkupObject* e,
...
CXTPMarkupObject* CXTPMarkupKeyboardNavigation::GetNextTabWithSameIndex(CXTPMarkupObject* e,
																		CXTPMarkupObject* container)
{
	int tabIndexHelper	   = GetTabIndexHelper(e);
	CXTPMarkupObject* obj2 = e;
...
CXTPMarkupObject* CXTPMarkupKeyboardNavigation::GetNextTabWithNextIndex(
	CXTPMarkupObject* e, CXTPMarkupObject* container, XTPMarkupKeyboardNavigationMode tabbingType)
{
	CXTPMarkupObject* obj2 = NULL;
...
CXTPMarkupObject* CXTPMarkupKeyboardNavigation::GetNextTabInGroup(
	CXTPMarkupObject* e, CXTPMarkupObject* container, XTPMarkupKeyboardNavigationMode tabbingType)
{
...
BOOL CXTPMarkupKeyboardNavigation::IsTabStop(CXTPMarkupObject* e)
{
	if (e->IsKindOf(MARKUP_TYPE(CXTPMarkupFrameworkElement)))
	{
		...
		return pElement->IsFocusable() && GetBoolValue(pElement, m_pIsTabStopProperty)
			   && pElement->IsEnabled() && pElement->IsVisible();
	}
	else if (e->IsKindOf(MARKUP_TYPE(CXTPMarkupFrameworkContentElement)))
	{
		return FALSE;
...
CXTPMarkupObject* CXTPMarkupKeyboardNavigation::GetNextTab(CXTPMarkupObject* e,
														   CXTPMarkupObject* pContainer,
														   BOOL goDownOnly)
{
	...
	else if (((keyNavigationMode == xtpMarkupKeyboardNavigationOnce)
			  || (keyNavigationMode == xtpMarkupKeyboardNavigationNone))
			 && (pContainer != e))
	{
		if (goDownOnly)
		{
	...
	if ((!goDownOnly && (tabbingType != xtpMarkupKeyboardNavigationContained))
		&& (GetParent(pContainer) != NULL))
	{
...
void CXTPMarkupKeyboardNavigation::Navigate(CXTPMarkupInputElement* pCurrentElement,
											XTPMarkupFocusNavigationDirection direction)
{
	...
	switch (direction)
	{
		case xtpMarkupFocusNavigationDirectionNext:
			...
			pNextInDirection = GetNextTab(pCurrentElement, GetGroupParent(pCurrentElement, TRUE),
										  FALSE);
			break;
...
void CXTPMarkupKeyboardNavigation::Navigate(CXTPMarkupInputElement* pSourceElement, UINT nChar)
{
	if (nChar == VK_TAB)
	{
		Navigate(pSourceElement, (::GetKeyState(VK_SHIFT) & 0x8000) == 0
