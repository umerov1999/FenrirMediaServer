...
#define TRY_POINTER_CALL(proc)                                                                     \
	...
	{                                                                                              \
		...
		{                                                                                          \
			sData.m_ptrWrappers[xtpWrapper##proc] = ::GetProcAddress(sData.m_hThemeDll,            \
																	 xtpWrapperProc##proc);        \
		}                                                                                          \
...
HRESULT CXTPWinThemeWrapper::DrawThemeBackground(HDC hDC, int iPartID, int iStateID,
...
HRESULT CXTPWinThemeWrapper::DrawThemeBackgroundEx(HDC hDC, int iPartID, int iStateID,
												   const RECT* pRect, const PDTBGOPTS pOptions)
{
...
HRESULT CXTPWinThemeWrapper::DrawThemeEdge(HDC hDC, int iPartID, int iStateID,
...
HRESULT CXTPWinThemeWrapper::DrawThemeIcon(HDC hDC, int iPartID, int iStateID, const RECT* pRect,
...
HRESULT CXTPWinThemeWrapper::DrawThemeText(HDC hDC, int iPartID, int iStateID, LPCWSTR pszText,
										   int iCharCount, DWORD dwTextFlags, DWORD dwTextFlags2,
										   const RECT* pRect)
{
	TRY_POINTER_CALL(DrawThemeText)
	return POINTER_CALL(m_hTheme, hDC, iPartID, iStateID, pszText, iCharCount, dwTextFlags,
						dwTextFlags2, pRect);
	CATCH_POINTER_CALL()
...
HRESULT CXTPWinThemeWrapper::GetCurrentThemeName(LPWSTR pszThemeFileName, int dwMaxNameChars,
												 ...
												 LPWSTR pszSizeBuff, int cchMaxSizeChars)
{
	...
	return POINTER_CALL(pszThemeFileName, dwMaxNameChars, pszColorBuff, cchMaxColorChars,
...
HRESULT CXTPWinThemeWrapper::GetThemeBackgroundContentRect(HDC hDC, int iPartID, int iStateID,
...
HRESULT CXTPWinThemeWrapper::GetThemeBackgroundExtent(HDC hDC, int iPartID, int iStateID,
...
HRESULT CXTPWinThemeWrapper::GetThemeBackgroundRegion(HDC hDC, int iPartID, int iStateID,
...
HRESULT CXTPWinThemeWrapper::GetThemeDocumentationProperty(LPCWSTR pszThemeName,
...
HRESULT CXTPWinThemeWrapper::GetThemeFilename(int iPartID, int iStateID, int iPropID,
...
HRESULT CXTPWinThemeWrapper::GetThemeFont(HDC hDC, int iPartID, int iStateID, int iPropID,
										  LOGFONTW* pFont)
{
...
HRESULT CXTPWinThemeWrapper::GetThemeIntList(int iPartID, int iStateID, int iPropID,
											 INTLIST* pIntList)
{
...
HRESULT CXTPWinThemeWrapper::GetThemeMargins(HDC hDC, int iPartID, int iStateID, int iPropID,
											 RECT* pRect, MARGINS* pMargins)
{
...
HRESULT CXTPWinThemeWrapper::GetThemeMetric(HDC hDC, int iPartID, int iStateID, int iPropID,
											int* piVal)
{
...
HRESULT CXTPWinThemeWrapper::GetThemePartSize(HDC hDC, int iPartID, int iStateID, RECT* pRect,
											  THEMESIZE eSize, SIZE* pSize)
{
...
HRESULT CXTPWinThemeWrapper::GetThemePropertyOrigin(int iPartID, int iStateID, int iPropID,
...
HRESULT CXTPWinThemeWrapper::GetThemeString(int iPartID, int iStateID, int iPropID, LPWSTR pszBuff,
											int cchMaxBuffChars)
{
...
HRESULT CXTPWinThemeWrapper::GetThemeSysString(int iStringID, LPWSTR pszStringBuff,
...
HRESULT CXTPWinThemeWrapper::GetThemeTextExtent(HDC hDC, int iPartID, int iStateID, LPCWSTR pszText,
...
HRESULT CXTPWinThemeWrapper::GetThemeTextMetrics(HDC hDC, int iPartID, int iStateID,
												 TEXTMETRIC* ptm)
{
...
HRESULT CXTPWinThemeWrapper::HitTestThemeBackground(HDC hDC, int iPartID, int iStateID,
													DWORD dwOptions, const RECT* pRect, HRGN hRgn,
													POINT ptTest, WORD* pwHitTestCode)
{
	TRY_POINTER_CALL(HitTestThemeBackground)
	return POINTER_CALL(m_hTheme, hDC, iPartID, iStateID, dwOptions, pRect, hRgn, ptTest,
						pwHitTestCode);
	CATCH_POINTER_CALL()
...
HRESULT CXTPWinThemeWrapper::DrawThemeTextEx(HDC hDC, int iPartId, int iStateId, LPCWSTR pszText,
											 int cchText, DWORD dwTextFlags, LPRECT pRect,
											 const XTP_UX_DTTOPTS* pOptions)
{
	TRY_POINTER_CALL(DrawThemeTextEx)
	return POINTER_CALL(m_hTheme, hDC, iPartId, iStateId, pszText, cchText, dwTextFlags, pRect,
						pOptions);
	CATCH_POINTER_CALL()
...
HRESULT CXTPWinThemeWrapper::BufferedPaintSetAlpha(HPAINTBUFFER hBufferedPaint, const RECT* prc,
												   BYTE alpha)
{
...
HPAINTBUFFER CXTPWinThemeWrapper::BeginBufferedPaint(HDC hdcTarget, const RECT* prcTarget,
													 XTP_UX_BUFFERFORMAT dwFormat,
													 XTP_UX_PAINTPARAMS* pPaintParams, HDC* phdc)
{
...
BOOL CXTPWinDwmWrapper::IsCompositionEnabled()
{
	...
	{
		sData.m_ptrWrappers[xtpWrapperDwmIsCompositionEnabled] =
			::GetProcAddress(sData.m_hDwmDll, xtpWrapperProcDwmIsCompositionEnabled);
	}

	PFNDwmIsCompositionEnabled ptr = (PFNDwmIsCompositionEnabled)
										 sData.m_ptrWrappers[xtpWrapperDwmIsCompositionEnabled];
	if (ptr)
	{
		BOOL bEnabled	= FALSE;
...
HRESULT CXTPWinDwmWrapper::DefWindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam,
										 LRESULT* plResult)
{
	CSharedData& sData = GetSharedData();
	if (sData.m_hDwmDll != NULL && sData.m_ptrWrappers[xtpWrapperDwmDefWindowProc] == NULL)
	{
		sData.m_ptrWrappers[xtpWrapperDwmDefWindowProc] =
			::GetProcAddress(sData.m_hDwmDll, xtpWrapperProcDwmDefWindowProc);
	}
...
HRESULT CXTPWinDwmWrapper::ExtendFrameIntoClientArea(HWND hWnd, int cxLeftWidth, int cyTopHeight,
													 int cxRightWidth, int cyBottomHeight)
{
	CSharedData& sData = GetSharedData();
	if (sData.m_hDwmDll != NULL
		&& sData.m_ptrWrappers[xtpWrapperDwmExtendFrameIntoClientArea] == NULL)
	{
		sData.m_ptrWrappers[xtpWrapperDwmExtendFrameIntoClientArea] =
			::GetProcAddress(sData.m_hDwmDll, xtpWrapperProcDwmExtendFrameIntoClientArea);
	}

	PFNDwmExtendFrameIntoClientArea ptr =
		(PFNDwmExtendFrameIntoClientArea)sData.m_ptrWrappers[xtpWrapperDwmExtendFrameIntoClientArea];
	if (ptr)
	{
...
void CXTPWinDwmWrapper::LogicalToPhysicalPoint(HWND hWnd, LPPOINT lpPoint)
{
	...
	static PFNLOGICALTOPHYSICALPOINT pfnLogicalToPhysicalPoint = NULL;

	if (NULL == pfnLogicalToPhysicalPoint)
	{
		...
		pfnLogicalToPhysicalPoint = XTPToFunctionPtr<PFNLOGICALTOPHYSICALPOINT>(
...
void CXTPWinDwmWrapper::SetProcessDPIAware()
{
	...
	static PFNSETPROCESSDPIAWARE pfnSetProcessDPIAware = NULL;

	if (NULL == pfnSetProcessDPIAware)
	{
		HMODULE hModule = GetModuleHandle(_T("user32.dll"));
		ASSERT(hModule);
		if (!hModule)
		...
		pfnSetProcessDPIAware = XTPToFunctionPtr<PFNSETPROCESSDPIAWARE>(
