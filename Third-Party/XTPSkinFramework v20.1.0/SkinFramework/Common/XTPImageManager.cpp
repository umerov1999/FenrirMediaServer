...
BOOL AFX_CDECL CXTPImageManagerIcon::IsPngBitmapMemory(const void* Buf, int Size)
{
	if (Size < 4)
		return FALSE;
	BYTE png_signature[4] = { 137, 80, 78, 71 };

	LPBYTE pBitmapInfoHeader = (LPBYTE)Buf;
	BOOL bPngBitmap = memcmp(pBitmapInfoHeader, &png_signature, 4) == 0;
	return (bPngBitmap);
}
...
HBITMAP AFX_CDECL CXTPImageManagerIcon::LoadPNGFromMemory(const void* Buf, int Size, BOOL* lbAlphaBitmap)
{
#ifdef _XTP_INCLUDE_GRAPHICLIBRARY

	CXTPGraphicBitmapPng bmpIcons;
	if (!bmpIcons.LoadFromMemory(Buf, Size))
		return NULL;
										...
										PBITMAPINFO& pBitmapInfo, UINT& nBitmapInfoSize)
{
	...
	TRY
	{
		...
		int nColorTableSize = bmi.bmiHeader.biBitCount == 4	  ? 16
							  : bmi.bmiHeader.biBitCount == 8 ? 256
															  : 3;
...
CXTPImageManagerIcon* CXTPImageManagerIconSet::CreateVectorIcon(UINT nWidth)
{
	...
	{
		pIcon = new CXTPImageManagerIcon(m_nID, this);
		m_mapIcons.SetAt(nWidth, pIcon);
	}
