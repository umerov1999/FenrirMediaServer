...
BOOL CXTPNotifyConnection::SendEvent(XTP_NOTIFY_CODE Event, WPARAM wParam, LPARAM lParam,
									 DWORD dwFlags)
{
	CSingleLock singleLock(GetDataLock(), TRUE);
	InternalAddRef();
	...
	for (i = nFirstLocalClientIndex; i <= nLastLocalClientIndex; i++)
	{
		...
		try
		{
			...
			{
				ASSERT(FALSE);
				TRACE(_T("CXTPNotifyConnection::SendEvent(Event = %d, wParam = %d, lParam = %d, ")
					  _T("dwFlags = %x) pSink = %x\n"),
		...
		{
			ASSERT(FALSE);
			TRACE(_T("EXCEPTION! CXTPNotifyConnection::SendEvent(Event = %d, wParam = %d, lParam ")
				  _T("= %d, dwFlags = %x)\n"),
...
BOOL CXTPNotifySinkImplMTMsgWnd::CreateWnd()
{
	...
	BOOL bCreated = CreateEx(0, pcszSimpleWndClass, XTP_NOTIFICATION_SINK_MT_MSGWND_NAME, WS_POPUP,
							 rcEmpty, NULL, 0);
	ASSERT(bCreated);
	return bCreated;
}
...
void CXTPNotifySink::OnEvent(XTP_NOTIFY_CODE dwNotifyCode, WPARAM wParam, LPARAM lParam,
							 DWORD /*dwFlags*/)
{
...
void CXTPNotifySinkMT::OnEvent(XTP_NOTIFY_CODE dwNotifyCode, WPARAM wParam, LPARAM lParam,
							   DWORD dwFlags)
{
	...
	if (dwFlags & xtpNotifyPostMessage)
	{
		...
		do
		{
			bPosted = PostMessage(xtp_wm_NotificationSinkMTOnEvent, NULL,
								  static_cast<LPARAM>(dwDataID));
			if (!bPosted && (dwFlags & xtpNotifyGuarantyPost))
			{
				if (!bTrace1)
				{
					TRACE(_T("WARNING!  CXTPNotifySinkBaseImplMT: PostMessage return FALSE. WAIT ")
						  _T("and retry. (ThreadID = %x, dataID = %d) \n"),
		...
		if (bPosted)
		{
			...
			if (bTrace1)
			{
				TRACE(_T("WARNING.*  CXTPNotifySinkBaseImplMT: Event is posted! (ThreadID = %x, ")
					  _T("dataID = %d) \n"),
					  GetCurrentThreadId(), dwDataID);
			}
		...
		else if (!(dwFlags & xtpNotifyGuarantyPost))
		{
			if (m_dwTraceFlag0 < 3)
			{
				TRACE(_T("WARNING!  CXTPNotifySinkBaseImplMT: PostMessage return FALSE - Event is ")
					  _T("skipped! (ThreadID = %x, dataID = %d) \n"),
					  GetCurrentThreadId(), dwDataID);
...
LRESULT CXTPNotifySinkMT::OnInterThreadEvent(WPARAM pEventData, LPARAM dwPostDataID)
{
	...
	if (pIthData)
	{
		CXTPNotifySink::OnEvent(pIthData->dwNotifyCode, pIthData->wParam, pIthData->lParam,
								pIthData->dwFlags);
	}
	else
	{
		...
		if (m_PostedEvents.Lookup((DWORD)dwPostDataID, ithData))
		{
			...
			CXTPNotifySink::OnEvent(ithData.dwNotifyCode, ithData.wParam, ithData.lParam,
									ithData.dwFlags);
		}
		else
		{
