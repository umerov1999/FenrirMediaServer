...
CXTPPropExchangeRegistry::CXTPPropExchangeRegistry(BOOL bLoading, HKEY hParentKey,
												   LPCTSTR lpszSection)
{
...
CXTPPropExchange* CXTPPropExchangeRegistry::GetSection(LPCTSTR lpszSection)
{
	...
	CXTPPropExchangeRegistry* pSection = new CXTPPropExchangeRegistry(IsLoading(), m_hSectionKey,
																	  lpszSection);
...
BOOL CXTPPropExchangeRegistry::IsPropertyExists(LPCTSTR pszPropName)
{
	...
	return RegQueryValueEx(m_hSectionKey, (LPTSTR)pszPropName, NULL, NULL, NULL, NULL)
		   == ERROR_SUCCESS;
}
...
BOOL CXTPPropExchangeRegistry::IsSectionExists(LPCTSTR lpszSection)
{
	...
	if (RegOpenKeyEx(m_hSectionKey, lpszSection, 0, REGSAM(m_bLoading ? KEY_READ : KEY_WRITE),
					 &hSectionKey)
		== ERROR_SUCCESS)
	{
...
BOOL CXTPPropExchangeRegistry::OnBeforeExchange()
{
	...
	if (!m_bInitialized)
	{
		...
		VERIFY(RegCreateKeyEx(m_hParentKey, m_strSectionName, 0, REG_NONE, REG_OPTION_NON_VOLATILE,
							  REGSAM(m_bLoading ? KEY_READ : KEY_WRITE), NULL, &m_hSectionKey, NULL)
			   == ERROR_SUCCESS);
...
BOOL CXTPPropExchangeRegistry::ExchangeProp(LPCTSTR pszPropName, VARTYPE vtProp, void* pvProp,
											const void* pvDefault /*= NULL*/)
{
	if (m_bLoading)
	{
		...
		switch (vtProp)
		{
			...
			case VT_VARIANT: return FALSE; // Not Implemented
			...
			case VT_EX_RECT:
			{
				...
				if (lResult == ERROR_SUCCESS)
				{
					...
					LPBYTE lpBuffer = (LPBYTE)strValue.GetBuffer(
						XTPToIntChecked(dwCount / sizeof(TCHAR)));
				...
				if (lResult == ERROR_SUCCESS)
				{
					...
					{
						RECT rc = { 0, 0, 0, 0 };
						if (SCANF_S(strValue, _T("%li, %li, %li, %li"), &rc.left, &rc.top,
									&rc.right, &rc.bottom)
							!= 4)
							return CopyPropValue(vtProp, pvProp, pvDefault);
	...
	else
	{
		...
		switch (vtProp)
		{
			...
			case VT_BOOL:
			{
				...
				lResult = RegSetValueEx(m_hSectionKey, pszPropName, NULL, REG_DWORD,
			...
			case VT_R4:
			case VT_R8:
				lResult = RegSetValueEx(m_hSectionKey, pszPropName, NULL, REG_BINARY,
										(LPBYTE)pvProp, GetSizeOfVarType(vtProp));
				break;
...
BOOL CXTPPropExchangeRegistry::ExchangeBlobProp(LPCTSTR pszPropName, BYTE*& pData, DWORD& nBytes)
{
	...
	if (IsStoring())
	{
		LONG lResult = ::RegSetValueEx(m_hSectionKey, pszPropName, NULL, REG_BINARY, pData, nBytes);
		return lResult == ERROR_SUCCESS;
	}
	else
	{
		DWORD dwType, dwCount;
		LONG lResult = RegQueryValueEx(m_hSectionKey, (LPTSTR)pszPropName, NULL, &dwType, NULL,
									   &dwCount);
		...
		lResult = RegQueryValueEx(m_hSectionKey, (LPTSTR)pszPropName, NULL, &dwType, (LPBYTE)pData,
								  &dwCount);
