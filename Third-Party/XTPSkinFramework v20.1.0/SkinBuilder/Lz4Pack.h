...
#pragma pack (pop)
	...
	public:
		...
		ComressorEntry(const std::vector<char>& uncompressed);
	...
	class Compressor {
	public:
		bool add(const std::wstring& file_name, const std::vector<char>& uncompressed);
		bool add(const std::wstring& file_name, const std::string& uncompressed);
		bool addDir(const std::wstring& file_name);
		void CheckAndMakeDirecories(const std::wstring& PathIgnoreFile);
		bool build(const std::wstring& arch);
	private:
		Map::Map<std::string, ComressorEntry> wrap;
	};
	...
	public:
		...
		Map::Map<std::wstring, DecomressorEntry> Files;
