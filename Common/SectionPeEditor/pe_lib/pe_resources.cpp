...
//auto_strip_last_section - if true and resources are placed in the last section, it will be automatically stripped
//number_of_id_entries and number_of_named_entries for resource directories are recalculated and not used
const image_directory rebuild_full_resources(pe_base& pe, resource_directory& info, uint32_t offset_from_section_start, bool save_to_pe_header, bool auto_strip_last_section)
{
	bool NeadRelocateTable = false;
	bool NotAddSection = false;
	section new_resources;
	new_resources.set_name(".rsrc");
	new_resources.get_raw_data().resize(1);
	new_resources.initializingdata(true);
	...
	if (pe.has_resources())
	{
		section &ress = pe.section_from_directory(pe_win::image_directory_entry_resource);
		new_resources.set_characteristics(ress.get_characteristics());

		if ((ress.Number != pe.get_number_of_sections() && pe.get_image_sections()[pe.get_number_of_sections() - 1].get_name() == ".reloc"
			&& ress.Number == pe.get_number_of_sections() - 1) || ress.Number == pe.get_number_of_sections())
		{
			NeadRelocateTable = ress.Number != pe.get_number_of_sections();
			if (NeadRelocateTable == true)
			{
				reld = pe.section_from_directory(pe_win::image_directory_entry_basereloc).get_raw_data();
				relc = pe.section_from_directory(pe_win::image_directory_entry_basereloc).get_characteristics();
				pe.get_image_sections().erase(pe.get_image_sections().end() - 1);
			}
			NotAddSection = true;
			ress.get_raw_data().resize(1);
		}
		else
		{
			ress.set_name("[dirty]");
			memset((char*)ress.get_raw_data().data(), 0, ress.get_raw_data().size());
		}
	}
	else if (pe.get_image_sections()[pe.get_number_of_sections() - 1].get_name() == ".reloc")
	{
		reld = pe.section_from_directory(pe_win::image_directory_entry_basereloc).get_raw_data();
		relc = pe.section_from_directory(pe_win::image_directory_entry_basereloc).get_characteristics();
		pe.get_image_sections().erase(pe.get_image_sections().end() - 1);
		NeadRelocateTable = true;
	}
	
	void* resources_section = NULL;
	if (NotAddSection == false)
	{
		resources_section = (void*)&pe.add_section(new_resources);
	}
	else
	{
		resources_section = (void*)&pe.section_from_directory(pe_win::image_directory_entry_resource);
	}
	//Check that resources_section is attached to this PE image
	if (!pe.section_attached((*(section*)resources_section)))
		throw pe_exception("Resource section must be attached to PE file", pe_exception::section_is_not_attached);
	...
	uint32_t needed_size = needed_size_for_structures + needed_size_for_strings + needed_size_for_data;
	std::string& raw_data = (*(section*)resources_section).get_raw_data();
	...
	uint32_t current_strings_pos = current_structures_pos + needed_size_for_structures;
	uint32_t current_data_pos = current_strings_pos + needed_size_for_strings;
	rebuild_resource_directory(pe, (*(section*)resources_section), info, current_structures_pos, current_data_pos, current_strings_pos, aligned_offset_from_section_start);

	//Adjust section raw and virtual sizes
	pe.recalculate_section_sizes((*(section*)resources_section), auto_strip_last_section);

	image_directory ret(pe.rva_from_section_offset((*(section*)resources_section), aligned_offset_from_section_start), needed_size);
	...
	if (save_to_pe_header)
	{
		...
		{
			section relocate;
			relocate.set_name(".reloc");
			relocate.set_raw_data(reld);
			relocate.set_characteristics(relc);
			
			section &relocat = pe.add_section(relocate);
			image_directory reloc(pe.rva_from_section_offset(relocat, aligned_offset_from_section_start), relocat.get_size_of_raw_data());
			pe.set_directory_rva(image_directory_entry_basereloc, reloc.get_rva());
			pe.set_directory_size(image_directory_entry_basereloc, reloc.get_size());
		}
		pe.set_directory_rva(image_directory_entry_resource, ret.get_rva());
		pe.set_directory_size(image_directory_entry_resource, ret.get_size());
	}
