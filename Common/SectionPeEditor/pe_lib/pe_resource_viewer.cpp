...
//Returns resource language by type, ID and index in language directory (instead of language)
uint32_t pe_resource_viewer::get_resource_language_data_by_id(resource_type type, uint32_t id, uint32_t index) const
{
		const resource_directory::entry_list& entries = root_dir_ //Type directory
			.entry_by_id(type)
			.get_resource_directory() //Name/ID directory
		...
		return entries.at(index).get_id(); //Data directory
}
