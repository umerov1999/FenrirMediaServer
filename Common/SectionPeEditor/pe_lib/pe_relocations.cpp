...
const image_directory rebuild_header_relocations(pe_base& pe, section& reloc_section)
{
	if (!pe.section_attached(reloc_section))
		throw pe_exception("Relocations section must be attached to PE file", pe_exception::section_is_not_attached);

	uint32_t start_reloc_pos = pe_utils::align_up(0, sizeof(uint32_t));

	image_directory ret(pe.rva_from_section_offset(reloc_section, start_reloc_pos), reloc_section.get_size_of_raw_data());
