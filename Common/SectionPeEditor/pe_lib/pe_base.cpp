...
bool pe_base::SectionExist(char* sectnm)
{
	//Search for section
	for (section_list::iterator i = sections_.begin(); i != sections_.end(); ++i)
	{
		...
		if (strcmp(i->get_name().data(), sectnm) == 0)
		{
			return true;
		}
	}
...
section& pe_base::find_section_by_name(char* name)
{
	//Search for section
	for (section_list::iterator i = sections_.begin(); i != sections_.end(); ++i)
	{
		section& s = *i;
		//Return section if found
		if (strcmp(s.get_name().data(), name) == 0)
			return s;
	}
...
//Returns section from RVA
const section& pe_base::find_section_by_name(char* name) const
{
	//Search for section
...
section& pe_base::add_section(section s)
{
	...
	{
		if (sections_[i].ImagePosition >= 0)
			sections_[i].ImagePosition = -1;
	}

	s.Number = (uint32_t)sections_.size() + 1;
...
void pe_base::read_pe(std::istream& file, bool read_debug_raw_data)
{
	...
	for(int i = 0; i < get_number_of_sections(); i++)
	{
		...
		if(s.get_size_of_raw_data() != 0)
		{
			...
			s.get_raw_data().resize(s.get_size_of_raw_data());
			s.ImagePosition = (int)file.tellg();
...
void pe_base::get_advanced_pe_version(uint16_t &MajorImageVersion, uint16_t &MinorImageVersion, uint32_t &Win32VersionValue, uint16_t &Subsystem) const
{
	props_->get_advanced_pe_version(MajorImageVersion, MinorImageVersion, Win32VersionValue, Subsystem);
}
